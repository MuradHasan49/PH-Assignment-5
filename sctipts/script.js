...
let displayContainer = (data) => {
    ...
    data.forEach(item => {
        ...
        if (validation) {
            ...
            opendiv.innerHTML = `
            <div onclick="modalFucntion(${item.id})"  class="card w-full h-[425px] bg-white shadow-md rounded-lg border-t-4  overflow-hidden ${colorStatus}">
                ...
                <div class=" flex justify-between">
                    <div class="flex justify-between">
                        <div>
                            ...
                            <span>by <br></span>
                            <span class="font-semibold text-slate-600">${item.author}</span>
                        </div>
                    </div>
                    <span class="text-slate-400 font-medium tracking-wide mt-[20px]">${item.createdAt}</span>

                </div>
                <div class="flex justify-between  mt-1">
                    <div>
                        <span>Assignee by <br></span>
                        <span class="font-semibold text-slate-600"> ${item.assignee || item.author}</span>
                    </div>
                    <div>
                    <span>Updated : <br></span>
                    <span class="text-slate-400 font-medium tracking-wide mt-[20px]">${item.updatedAt ||
                item.createdAt}</span>
                    </div>
                </div>
            </div>
